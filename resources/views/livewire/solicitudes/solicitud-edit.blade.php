        ...
        <x-slot name="content">
            <div class="flex flex-wrap max-h-[320px] overflow-y-auto">
                @if (Auth::user()->permiso_id == 1 || Auth::user()->permiso_id == 4)
                    <span
                        class=" text-purple-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-purple-900 dark:text-purple-300">{{ __('Adjuntar Información:') }}</span>
                        ...
                        @if (Auth::user()->permiso_id == 1 || Auth::user()->permiso_id == 4)
                            <div class="mb-3 mr-2 ">
                                ...
                                <x-input wire:model="total"
                                    ...
                                    type="number" name="total" :value="old('total')" required autofocus
                                    autocomplete="total"
                ...
                @endif
                <span
                    class=" text-purple-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-purple-900 dark:text-purple-300">{{ __('Añadir Producto:') }}</span>
                ...
                <div class="w-full">
                    <span
                        class=" text-purple-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-purple-900 dark:text-purple-300">{{ __('Productos Solicitados:') }}</span>
                </div>
                ...
                <div class="w-full">
                    <div>
                        @if (in_array($solicitudEs->categoria_id, [5, 6, 7]))
                            <div class="mb-1 col-12 w-full" x-data="{ isUploading: false, progress: 0 }"
                                x-on:livewire-upload-start="isUploading = true"
                                x-on:livewire-upload-finish="isUploading = false"
                                x-on:livewire-upload-error="isUploading = false"
                                x-on:livewire-upload-progress="progress = $event.detail.progress">

                                <x-label value="{{ __('Evidencias') }}"
                                    class="border-b border-gray-400 w-full text-left mb-2" />
                                <input type="file" wire:model="evidencias"
                                    class=" pb-2 flex flex-wrap file:text-sm file:font-semibold file:bg-blue-300 file:text-blue-700 hover:file:bg-blue-100 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0"
                            ...
                            <div class="flex flex-wrap gap-3 py-2">
                                @foreach ($solicitudEs->evidencias as $antigArch)
                                    @if ($antigArch->flag_trash == 0)
                                        <div
                                            class="p-5 relative max-w-[18rem] border rounded-md shadow-md dark:bg-slate-700 dark:border-slate-700">
                                            @if (
                                                ...
                                                <a href="{{ asset('storage/' . $antigArch->archivo_path) }}"
                                                    ...
                                                    <figure class="d-inline-block max-w-[160px]" tabindex="0"
                                                        ...
                                                        <img class="w-full" style="width:100px;"
                                            ...
                                            <button type="button" class="absolute top-1 right-1"
                                                wire:click="deleteEvidencia({{ $antigArch->id }})"
                                                data-bs-toggle="tooltip" data-bs-placement="top"
